...
class ForwardViewFeatureNet(nn.Module):
    ...
    def update_values(self, voxel_size, pc_range):
        # Need pillar (voxel) size and x/y offset in order to calculate pillar offset
        self.vx = voxel_size[:, 0]
        self.vy = voxel_size[:, 1]
        self.x_offset = self.vx / 2 + pc_range[:, 0]
    ...
    def forward(self, features, num_voxels, coors, batch_size):
        ...
        for batch_itt in range(batch_size):
            ...
            f_center[batch_mask, :, 0] = f_center[batch_mask, :, 0] - (this_coors[:, 3].float().unsqueeze(1) * self.vx[batch_itt] + self.x_offset[batch_itt])
            f_center[batch_mask, :, 1] = f_center[batch_mask, :, 1] - (this_coors[:, 2].float().unsqueeze(1) * self.vy[batch_itt] + self.y_offset[batch_itt])
