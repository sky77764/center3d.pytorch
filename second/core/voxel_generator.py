...
class VoxelGenerator:
    def __init__(self,
        ...
        point_cloud_range = np.array([0, 0, 0, 1.5708, 0.37, 79.756], dtype=np.float32)
        voxel_size = point_cloud_range[3:] / grid_size
    ...
    def generate(self, points, max_voxels):
        voxels, coors, num_points_per_voxel, self._phi_min, self._theta_min, self._voxel_size, self._point_cloud_range = points_to_voxel(
            points, self._grid_size, self._voxel_size, self._point_cloud_range,
            self._max_num_points, True, max_voxels, self._spherical_coor)
